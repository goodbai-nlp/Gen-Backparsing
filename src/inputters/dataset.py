...
def get_source_fields(fields=None):
    ...
    fields["src"] = torchtext.data.Field(
        pad_token=Constants.PAD_WORD, eos_token=Constants.EOS_WORD, include_lengths=True
    )  # include_lengths字段为True可以在返回minibatch的时候同时返回一个表示每个句子长度的list
...
def get_target_fields(fields=None):
    ...
    fields["tgt"] = torchtext.data.Field(
        init_token=Constants.BOS_WORD, eos_token=Constants.EOS_WORD, pad_token=Constants.PAD_WORD
    )
...
def get_structure_fields(fields=None):
    ...
    nesting_field = torchtext.data.Field(pad_token=Constants.PAD_WORD)
...
def get_relation_fields(fields=None):
    ...
    fields["relation"] = torchtext.data.Field(
        sequential=True, pad_token=Constants.PAD_WORD, unk_token=Constants.UNK_WORD
    )
...
def get_relation2_fields(fields=None):
    if fields is None:
        fields = {}

    nesting_field = torchtext.data.Field(pad_token=Constants.PAD_WORD, unk_token=Constants.UNK_WORD)
    fields["relation2"] = torchtext.data.NestedField(nesting_field, pad_token=Constants.PAD_WORD)
...
def get_align_fields(fields=None):
    if fields is None:
        fields = {}

    nesting_field = torchtext.data.Field(pad_token=Constants.PAD_WORD)

    fields["align"] = torchtext.data.NestedField(nesting_field, pad_token=Constants.PAD_WORD)
...
def get_align_gold_fields(fields=None):
    if fields is None:
        fields = {}

    fields["align_gold"] = torchtext.data.Field(
        init_token=Constants.BOS_WORD, eos_token=Constants.EOS_WORD, pad_token=Constants.PAD_WORD
    )
...
def get_fields():
    ...
    fields = get_relation2_fields(fields)
    fields = get_align_fields(fields)
...
def load_fields(opt, checkpoint):
    ...
    fields["relation2"].nesting_field.vocab = fields["relation2"].vocab
    fields["align"].nesting_field.vocab = fields["align"].vocab
    logger.info(
        " * vocabulary size. source = %d; target = %d; structure = %d; mask = %d; relation = %d; relation2=%d; align = %d"
        % (
            len(fields["src"].vocab),
            len(fields["tgt"].vocab),
            len(fields["structure"].nesting_field.vocab),
            len(fields["mask"].vocab),
            len(fields["relation"].vocab),
            len(fields["relation2"].nesting_field.vocab),
            len(fields["align"].vocab),
        )
    )
            ...
            def _pool(data, random_shuffler):
                for p in torchtext.data.batch(data, self.batch_size * 100):
                    p_batch = torchtext.data.batch(
                        sorted(p, key=self.sort_key), self.batch_size, self.batch_size_fn
                    )
...
def build_dataset(
    ...
    if structure_data_iter != None:
        structure_examples_iter = Dataset.make_nested_examples(
            structure_data_iter, None, "structure"
        )
    else:
    ...
    if relation_data_iter_2 != None:
        relation_examples_iter_2 = Dataset.make_nested_examples_new(
            relation_data_iter_2, None, "relation2"
        )
    else:
        relation_examples_iter_2 = None

    if align_data_iter != None:
        align_examples_iter = Dataset.make_nested_examples_new(align_data_iter, None, "align")
    else:
        align_examples_iter = None

    dataset = Dataset(
        fields,
        src_examples_iter,
        tgt_examples_iter,
        structure_examples_iter,
        mask_examples_iter,
        relation_examples_iter,
        relation_examples_iter_2,
        align_examples_iter,
...
class Dataset(torchtext.data.Dataset):
    def __init__(
        ...
        if (
            tgt_examples_iter is not None
            and structure_examples_iter is not None
            and mask_examples_iter is not None
            and relation_examples_iter is not None
            and relation_examples_iter_2 is not None
            and align_examples_iter is not None
        ):
            examples_iter = (
                _join_dicts(src, tgt, structure, mask, relation, relation2, align)
                for src, tgt, structure, mask, relation, relation2, align in zip(
                    src_examples_iter,
                    tgt_examples_iter,
                    structure_examples_iter,
                    mask_examples_iter,
            ...
            out_fields = get_relation2_fields(out_fields)
            out_fields = get_align_fields(out_fields)
        else:
            # examples_iter = src_examples_iter
            if tgt_examples_iter is None:
                examples_iter = (
                    _join_dicts(src, structure)
                    for src, structure in zip(src_examples_iter, structure_examples_iter)
                )
            else:
                examples_iter = (
                    _join_dicts(src, tgt, structure)
                    for src, tgt, structure in zip(
                        src_examples_iter, tgt_examples_iter, structure_examples_iter
                    )
                )
                out_fields = get_target_fields(out_fields)
            out_fields = get_structure_fields(out_fields)
        ...
        for ex_values in example_values:
            ...
            for (name, field), val in zip(out_fields, ex_values):
                if field is not None:
                    setattr(
                        example, name, field.preprocess(val)
    ...
    @staticmethod
    def make_nested_examples_new(text_iter, truncate, side):
        for i, line in enumerate(text_iter):
