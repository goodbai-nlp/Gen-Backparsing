...
cate=dev
setting=post2
vocab_dir=LDC2017-10000-60-5000-$setting

# modellist=(0.1-0.0-1.0-integrated-catall-0.1-0.4-0.5)
modellist=(0.1-0.1-1.0-integrated-catall-0.5-0.5-0.5-embnew)

dev=0
for model in ${modellist[@]}
do
tag=LDC2017-$model-$setting
base_dir=/home2/xfbai/mywork/AMR2Text/AMR-Backparsing
train_test_data_dir=$base_dir/data/LDC2017-$setting
model_file=./workspace/LDC2017-4096-10000-$setting-$model/_step_
reference="$train_test_data_dir/${cate}.sent"
output_dir='./workspace/translate-result'
if [ ! -d "$output_dir" ]; then mkdir -p "$output_dir"; fi
hypothesis=$output_dir/${cate}.${tag}
bleu_result="./workspace/LDC2017-4096-10000-$setting-$model/${cate}.${tag}"
...
CUDA_VISIBLE_DEVICES=$dev python3  ./translate.py -model  $model_file$step.pt \
                                                 -data ./workspace/$vocab_dir/LDC2017-$setting \
