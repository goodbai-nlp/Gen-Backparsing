...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, key, value, query, structure=None, mask=None,
        ...
        con_attns = attn.view(batch_size, head_count, query_len, key_len).contiguous()
        avg_attn = torch.mean(con_attns, dim=1)        # [bsz * query_len * key_len]
        if mask is not None:
            # print('mask', mask.size())
            avg_attn = avg_attn.masked_fill(mask.squeeze(1), -1e18)

        return output, top_attn, avg_attn
