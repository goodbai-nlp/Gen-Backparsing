...
def main(opt, device_id):  # device_id=0 only 1 GPU
    ...
    R2S_idx_lst = [ relation2structure[str(i)] for i in range(len(fields['relation'].vocab))]
    device = torch.device("cuda:0" if use_gpu(opt) else "cpu")
    R2S_idx_Tensor = torch.from_numpy(np.array(R2S_idx_lst)).long().to(device)

    trainer.train(train_iter_fct, valid_iter_fct, opt.train_steps, opt.valid_steps, opt.ratio_alpha, opt.ratio_beta, opt.ratio_nmt, R2S=R2S_idx_Tensor)
